"""Caso de uso para desactivar un user."""
...
class DeactivateUser:
    """Caso de uso para desactivar un user del sistema."""
    ...
    def execute(
        ...
        """
        Desactiva un user del sistema.
        
        Args:
            target_user_identification: Document del user a desactivar
            admin_user_identification: Document del admin que ejecuta la accion
            reason: Razon opcional de la desactivacion
            
        Returns:
            User: User desactivado
            
        Raises:
            HTTPException: Si no se encuentra el user o no hay permisos
        """
        logger.info(f"Iniciando desactivacion de user: {target_user_identification}")
        ...
        if admin_user.role not in [RoleEnum.MANAGER, RoleEnum.SUPER_ADMIN]:
            raise HTTPException(
                ...
                detail="No tiene permisos para desactivar users"
            )
        ...
        if target_user.role not in [RoleEnum.EMPLOYEE, RoleEnum.SUPERVISOR]:
            raise HTTPException(
                ...
                detail="Solo se pueden desactivar users con rol EMPLOYEE o SUPERVISOR"
            )
        
        # 5. Verificar que el user no este ya desactivado
        if not target_user.is_active:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="El user ya esta desactivado"
            )
        
        # 6. Desactivar el user (esto genera el evento de dominio)
        target_user.deactivate(changed_by=admin_user, reason=reason)
        ...
        logger.info(f"User {target_user_identification} desactivado exitosamente")
