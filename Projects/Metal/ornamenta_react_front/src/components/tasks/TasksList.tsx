...
export const TasksList: React.FC<TasksListProps> = ({ userId: propUserId }) => {
  ...
  useEffect(() => {
    ...
    loadEmployees();
    loadWorks();
  }, [stateFilter, propUserId, startDate, endDate]);
  ...
  const loadTasks = async () => {
    try {
      ...
      if (propUserId) {
        response = await taskService.getTasksByUserId(propUserId, params);
      } else {
      ...
      setError(err instanceof Error ? err.message : 'Error al cargar las tasks');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      {items.length === 0 ? (
        <Card className="border-1 border-gray-200">
          <CardBody className="text-center py-10">
            <p className="text-gray-500">No hay tasks registradas</p>
          </CardBody>
        </Card>
      ) : (
        <TasksTable
